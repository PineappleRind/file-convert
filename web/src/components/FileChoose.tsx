...
interface FileChooseProps {
  signal: Setter<File | null>;
}

export default ({ signal }: FileChooseProps) => {
  const [file, setFile] = createSignal<File | null>(null);
  let fileInput;

  function updateFile(files: FileList | null) {
    const file = files && files[0];
    if (file && !validateFile(file)) return alert("This file type is not accepted");
    signal(file);
    setFile(file);
  }
    ...
    <div class={styles.fileChoose}>
      ...
      >
        ...
        <div style={{ display: "flex", "align-items": "center" }}>
          ...
          <div onClick={() =>  updateFile(null)} class={styles.removeFile}>&times;</div>
        </div>
      </Show>
      <input
        type="file"
        id="fileUpload"
        onInput={(e: InputEvent) => updateFile((e.target as HTMLInputElement).files)}
